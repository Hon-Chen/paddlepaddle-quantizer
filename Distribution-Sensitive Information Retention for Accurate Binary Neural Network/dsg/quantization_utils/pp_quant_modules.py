...
from .pp_quant_utils import *
import sys
from paddle.nn import Layer as ppModule
from paddle.nn import ParameterList as ppParameter
import paddle as torch

class QuantAct(ppModule):
	"""
	...
	def __init__(self,
		...
		self.register_buffer('beta', torch.to_tensor([beta]))
...
class Quant_Linear(ppModule):
	...
	def forward(self, x):
		...
		try:
			...
			w_min = x_transform.min(axis=1)
			w_max = x_transform.max(axis=1)
		if not self.full_precision_flag:
...
class Quant_Conv2d(ppModule):
	...
	def set_param(self, conv):
		try:
			...
			self.weight = conv.weight.clone()
			try:
				self.bias = conv.bias.clone()
			except AttributeError:
				self.bias = None
