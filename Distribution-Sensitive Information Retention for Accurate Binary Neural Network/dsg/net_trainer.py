...
class Trainer(object):
    ...
    def hook_teacher_feature(self, module, input, output):
        input = input[0]
        self.teacher_feature.append(input)
    
    def hook_student_feature(self, module, input, output):
        input = input[0]
        self.student_feature.append(input)
    ...
    def train(self, epoch):
        ...
        if epoch==0:
            for m in self.model_teacher.modules():
                if isinstance(m, nn.BatchNorm2d):
                    #m.register_forward_hook(self.hook_teacher_feature)
                    m.register_forward_hook(self.hook_fn_forward)
            #for m in self.model.modules():
            #    if isinstance(m, nn.BatchNorm2d):
            #        m.register_forward_hook(self.hook_student_feature)
        ...
        for i in range(iters):
            ...
            self.student_feature.clear()
            self.teacher_feature.clear() 
            ...
            layer_num = 20 
            mn_perc = 10
            m = np.load('./m_n/m_' + str(mn_perc) + '.npy', allow_pickle = True) 
            n = np.load('./m_n/n_' + str(mn_perc) + '.npy', allow_pickle = True)          
            ...
            #for i in range(0, len(self.teacher_feature)):
            print("dyf debug 1: loss_distill")
                loss_distill += self.attention_loss(self.teacher_feature[i], self.student_feature[i])                
                #loss_distill += self.MSE_loss(self.teacher_feature[i], self.student_feature[i])
            #print(loss_S.size(), loss_distill.size())
            #loss_S += loss_distill.item()
