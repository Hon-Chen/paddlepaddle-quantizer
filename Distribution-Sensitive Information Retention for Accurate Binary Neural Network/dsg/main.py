...
from quantization_utils.pp_quant_modules import *
import torch
from pytorchcv_local.model_provider import get_model as ptcv_get_model
    ...
    def __init__(self, options=None, conf_path=None):
        ...
        self.conv_blocks2 = nn.Sequential(
            nn.Conv2d(128, 64, 3, stride=1, padding=1), nn.BatchNorm2d(
                64, 0.8), nn.LeakyReLU(0.2, inplace=True),
...
class ExperimentDesign:
    ...
    def export(self,pp_model):
        from paddle.static import InputSpec
        paddle_model = pp_model
        paddle.jit.save(paddle_model, "checkpoint",
                        input_spec=[InputSpec(shape=[1, 3, 224, 224], dtype="float32")])
        paddle.onnx.export(paddle_model, 'ckpt', 
                           input_spec=[InputSpec(shape=[1, 3, 224, 224], dtype="float32")])
        os.rename('ckpt.onnx', 'ckpt.pdiparams')
    ...
    def prepare(self):
        ...
        self._change_paddle()
        self.logger.info("self._change_paddle() success!")
    ...
    def _change_paddle(self):
        # torch_model = self.model
        import paddle
        paddle_model = paddle.vision.models.resnet18(pretrained=True)
        from paddlenlp.transformers.bert.mo import 
        paddle_model = 
        # paddle_model = paddle.vision.models.resnet101(pretrained=True)
        # squeezenet1_0
        self.export(pp_model=paddle_model)

        print('save epoch: 00')
        # for name, param in torch_model.named_parameters():
        #     paddle_model.set_state_dict({name: paddle.to_tensor(param.detach().numpy())})
        self.model = paddle_model
    ...
    def _set_model(self):
        if self.settings.dataset in ["cifar100"]:
            ...
            print(self.arc)
            if self.arc == 18:
                self.model = ptcv_get_model('resnet18', pretrained=True) # stuck
                self.model_teacher = ptcv_get_model('resnet18',
                                                    pretrained=True)
            elif self.arc == 50:
                print('resnet50')
                self.model = ptcv_get_model('resnet50', pretrained=True)
                self.model_teacher = ptcv_get_model('resnet50',
                                                    pretrained=True)
            self.model_teacher.eval()
    ...
    def _set_trainer(self):
        ...
        self.trainer = Trainer(model=self.model,
                               ...
                               arc=self.arc,
                               mn_perc=self.mn_perc)

    def quantize_model(self, model):
        import paddle.nn.layer as pl
        import paddle
        """
        ...
        if type(model) in (nn.Conv2d, pl.Conv2D):
            quant_mod = Quant_Conv2d(weight_bit=weight_bit)
            quant_mod.set_param(model)
            return quant_mod
        elif type(model) in (nn.Linear, pl.Linear):
        ...
        elif type(model) == pl.ReLU or type(model) == pl.ReLU6:
            return paddle.nn.Sequential(*[model, QuantAct(activation_bit=act_bit)])
        ...
        # recursively use the quantized module to replace the single-precision module
        ...
        elif type(model) == paddle.nn.Sequential:
            mods = []
            for n, m in model.named_children():
                mods.append(self.quantize_model(m))
            ...
            for attr in dir(model):
                ...
                if (isinstance(mod, nn.Module) or isinstance(mod, paddle.nn.Layer)) and 'norm' not in attr:
        ...
        try:
            for epoch in range(self.start_epoch, self.settings.nEpochs):
                ...
                with paddle.no_grad():
                    ...
                    if epoch > 0:
                        # layer_state_dict = self.model.state_dict()
                        # paddle.save(layer_state_dict, "model.pdparams")
                        # opt_state_dict = self.trainer.optimizer_S.state_dict()
                        # paddle.save(opt_state_dict, "model.pdopt")

                        # from paddle.static import InputSpec
                        # paddle.jit.save(self.model, "checkpoint",
                        #                 input_spec=[InputSpec(shape=[1,3,224,224], dtype="float32")])
